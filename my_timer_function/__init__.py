...
def configure_ssl_certificate():
    """
    Pobiera certyfikat SSL z zmiennej środowiskowej `CERT`, zapisuje go w pliku tymczasowym
    i ustawia zmienną środowiskową `SSL_CERT_FILE` na ścieżkę do tego pliku.
    """
    try:
        cert_content = os.getenv("CERT")
        if not cert_content:
            logging.warning("Zmienna środowiskowa `CERT` nie jest ustawiona. Połączenia HTTPS mogą nie działać.")
            return
        cert_path = "/tmp/azure_cert.pem"
        with open(cert_path, "w") as cert_file:
            cert_file.write(cert_content)
        os.environ["SSL_CERT_FILE"] = cert_path
        logging.info(f"Certyfikat SSL skonfigurowany: {cert_path}")
    except Exception as e:
        logging.error(f"Błąd podczas konfiguracji certyfikatu SSL: {e}")


# Konfiguracja certyfikatu przy starcie aplikacji
